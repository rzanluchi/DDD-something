...
class MessageDispatcher(object):
    ...
    def add_handler_for(self, command, handler):
        ...
        """
        if not self.command_handlers.get(command):
    ...
    def add_subscriber_for(self, event, subscriber):
        ...
        """
        if not self.event_subscribers.get(event):
    ...
    def _execute_command(self, handler, command):
        ...
        aggregate.apply_events(past_events)
        new_events = aggregate.handle(command)
        self._store_events_for(aggregate, new_events)
